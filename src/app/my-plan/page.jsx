      ...
      <section className="container mx-auto">
        <div className="mx-5 mt-10">
          ...
          <div className="mt-6 flex items-center justify-between">
            <div className="flex w-fit items-center rounded-xl border border-[#202631] bg-[#11141b] p-1">
              {tabs.map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`h-7 min-w-24 rounded-lg px-4 text-xs ${activeTab === tab
                      ? "bg-[#252b37] font-semibold text-white shadow-sm"
                      : "text-neutral-500"
                    }`}
                >
                  {tab}
            ...
            {loading ? (
              ...
              <ul className="w-full  flex flex-col gap-3 items-center justify-center">
                {sortedList.map((item, idx) => (
                  <MyPlanCard key={idx} currentPlan={item} onMarkAsDone={() => handleMarkAsDone(item)} onDismiss={() => handleDismiss(item)} />
                ))}
              </ul>
            ) : (
              <EmptyCard />
